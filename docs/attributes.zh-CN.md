...
规范目录是 `src/features/attributes/`，公开入口为 `index.ts`。模块负责表格展示、列与行、虚拟化滚动、排序、搜索/选择筛选入口及行选择交互。
...
│  └─ AttributeTableHeader.tsx   # 搜索、已选、清除及打开图表入口
├─ stores/
...
本阶段没有增加单元格编辑、字段增删改、SQL、服务端分页或表格状态持久化。涉及真实属性更新时应继续接入数据拥有方的统一接口，而不是在本模块建立数据仓库。
