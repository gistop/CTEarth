...
describe('independent raster rendering', () => {
  it('keeps both MapLibre rasters, reuses sources, hides B and removes only deleted resources', () => {
    ...
    syncMapLibreRasters(map, [a], {}, {}, true);
    const sourceA = sources['raster-overlay-A'];
    syncMapLibreRasters(map, [b, a], {}, {}, true);
    expect(layers.size).toBe(2);
    expect(sources['raster-overlay-A']).toBe(sourceA);
    syncMapLibreRasters(map, [b, a], { B: false }, { A: { opacity: 0.4 } }, true);
    ...
    expect(api.setPaintProperty).toHaveBeenCalledWith('raster-overlay-B', 'raster-opacity', 0.82);
  ...
  it('maintains separate OpenLayers images, visibility, order and cleanup', () => {
    ...
    syncOpenLayersRasters(map, layers, [a], {}, {}, true, 'EPSG:4326');
    const sourceA = layers.get('A')!.getSource();
    syncOpenLayersRasters(map, layers, [b, a], { B: false }, { A: { opacity: 0.5 } }, true, 'EPSG:4326');
    expect(layers.size).toBe(2);
    ...
    expect(layers.get('B')!.getOpacity()).toBe(0.82);
    ...
    syncOpenLayersRasters(map, layers, [], {}, {}, true, 'EPSG:4326');
    expect(layers.size).toBe(0);
  });
  ...
  it('renders all Cesium rasters in draw order and preserves A when B is hidden or deleted', async () => {
    ...
    const request = {
      ...
      rasterLayerVisibility: {}, rasterStyles: { A: { opacity: 0.6 }, B: { opacity: 0.6 } }, layers: [],
